...
int main() {
    ...
    int total_frames = 0;
    int ip_frames = 0, arp_frames = 0, other_eth = 0;
    int ip_total = 0;
    int tcp_count = 0, udp_count = 0, icmp_count = 0, other_ip = 0;
    ...
    while (total_frames < 1000) {
        n = recvfrom(s, buffer, sizeof(buffer), 0, (struct sockaddr *)&sll, &len);
        if (n == -1) {
            perror("Recvfrom Failed");
            return 1;
        }
        ...
        unsigned short eth_type = ntohs(*(unsigned short *)(buffer + 12));
        if (eth_type == 0x0800) { // IP
            ip_frames++;
            struct ip_datagram *ip = (struct ip_datagram *)(buffer + 14);
            ip_total++;
            switch (ip->proto) {
                case 6: tcp_count++; break;
                case 17: udp_count++; break;
                case 1: icmp_count++; break;
                default: other_ip++; break;
            }
    ...
    printf("--- Ethernet Frame Statistics ---\n");
    printf("IP frames: %.2f%%\n", 100.0 * ip_frames / total_frames);
    printf("ARP frames: %.2f%%\n", 100.0 * arp_frames / total_frames);
    printf("Other frames: %.2f%%\n", 100.0 * other_eth / total_frames);

    printf("\n--- IP Packet Statistics ---\n");
    if (ip_total > 0) {
        printf("TCP packets: %.2f%%\n", 100.0 * tcp_count / ip_total);
        printf("UDP packets: %.2f%%\n", 100.0 * udp_count / ip_total);
        printf("ICMP packets: %.2f%%\n", 100.0 * icmp_count / ip_total);
        printf("Other IP packets: %.2f%%\n", 100.0 * other_ip / ip_total);
    } else {
        printf("No IP packets captured.\n");
    }
