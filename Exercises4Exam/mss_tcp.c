...
unsigned char * print_flags(unsigned short flags){
        switch (flags){
                case SYN:
                        return "SYN";
                case ACK:
...
txcb->retry = 0;
//Resize the segment according to MSS
tcp = txcb->segment = (struct tcp_segment *) malloc(sizeof(struct tcp_segment) - TCP_MSS + t->mss);
        ...
        case SYN_SENT:
                if(event == PKT_RCV){
                        if((tcp->flags&SYN) && (tcp->flags&ACK) && (htonl(tcp->ack)==tcb->seq_offs + 1)){
                                //We received an ACK, adjust the mss
                                unsigned short mss = TCP_MSS;
                                if(tcp->payload[0] == 2 && tcp->payload[1] ==4){
                                        mss = ntohs(tcp->payload[2]);
                                        printf("Received remote MSS: %d\n", mss);
                                }
                                //Set the new mss
                                tcb->mss = mss;
                                tcb->seq_offs ++;
  ...
  break;
         ...
         case  CLOSE_WAIT:
          ...
          break;
                ...
                break;
case LISTEN:
  //Someone wants to connect, check if he sent an mss and set it
        ...
        if(fdinfo[i].st == TCB_CREATED){
                ...
                for(tot=0,txcb=tcb->txfirst;  txcb!=NULL  /*&& (tot<tcb->radwin)*/;  txcb = txcb->next){
                  ...
                  if(txcb->txtime+tcb->timeout > tick )  continue; //No timeout
                        ...
                        //Cast TCP segment
                        struct tcp_segment *tcp = txcb->segment;
                        printf("OUT %s %d\n", print_flags(tcp->flags), (unsigned int) txcb->totlen);
                        printf("MSS OUT: %d\n", tcb->mss);
                ...
                if (eth->type == htons (0x0806)) {
                        ...
                        break;
                          if(i<MAX_FD)
                                                ...
                                                //Print inbound package
                                                printf("IN %s %u\n", print_flags(tcp->flags), htons(ip->totlen) - (ip->ver_ihl&0xF)*4);
                                                printf("MSS IN: %d\n",tcb->mss);
                                                printf("%.7ld: RX SOCK:%d ACK %d SEQ:%d SIZE:%d FLAGS:0x%.2X\n",rtclock(0),i,htonl(tcp->ack)-tcb->seq_offs,htonl(tcp->seq)-tcb->ack_offs,  htons(ip->totlen) - (ip->ver_ihl&0xF)*4 - (tcp->d_offs_res>>4)*4, ntohs(tcp->flags));
