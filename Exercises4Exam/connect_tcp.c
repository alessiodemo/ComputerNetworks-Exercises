...
int unique_s;
int fl;
unsigned char is_unreachable = 0;
...
if((addr->sa_family == AF_INET)){
        ...
        if ( s >= 3 && s<MAX_FD){
                        ...
                        while(sleep(10)){
                                        ...
                                        if(is_unreachable){
                                                printf("Destination %s unreacheable\n", is_unreachable == 1 ? "host" : "net" );
...
if (fds[0].revents & POLLIN){
        ...
        while ( 0 <= (size = recvfrom(unique_s,eth,MAXFRAME,0, (struct sockaddr *) &sll,&len))){
                ...
                if (eth->type == htons (0x0806)) {
                        ...
                        else if(ip->proto == 1 && !memcmp(&ip->dstaddr, myip, 4)){
...
addr.sin_addr.s_addr = inet_addr("50.87.68.238");// www.midor.com.eg
